...
export default function VoiceChatPage() {
  ...
  const [globalPeers, setGlobalPeers] = useState<{ [key: string]: RTCPeerConnection }>({});
  const [signalingChannel, setSignalingChannel] = useState<WebSocket | null>(null);
  ...
  const createPeerConnection = (userId: string, isInitiator: boolean) => {
    ...
    peerConnection.onicecandidate = (event) => {
      if (event.candidate) {
        ...
        signalingChannel?.send(JSON.stringify({ type: "candidate", candidate: event.candidate, to: userId }));
    ...
    peerConnection.ontrack = (event) => {
      ...
      const checkAudioLevel = () => {
          ...
          const volume = buffer.reduce((sum, val) => sum + val, 0) / buffer.length;
          if (!volume)
            return;
  ...
  if (isInitiator) {
    peerConnection.createOffer()
        ...
        .then(() => {
            signalingChannel?.send(JSON.stringify({ type: "offer", offer: peerConnection.localDescription, to: userId }));
  ...
  useEffect(() => {
    ...
    console.log("Канал веб-сокетів успішно ініціалізовано. Ініціалізація обробників...")
      ...
      if (data.type === "user-list") {
          ...
          data.users.forEach((userId : string) => {
            if (!peers[userId]) {
              ...
              if (peers[userId].iceConnectionState === "disconnected" || 
          ...
          signalingChannel.send(JSON.stringify({ type: "answer", answer, to: data.from }));
          console.log("📨 Надіслано answer");
      } else if (data.type === "answer") {
          await globalPeers[data.from].setRemoteDescription(new RTCSessionDescription(data.answer));
          console.log("✅ Отримано answer");
      } else if (data.type === "candidate") {
          await globalPeers[data.from].addIceCandidate(new RTCIceCandidate(data.candidate));
          console.log("🎯 Отримано кандидата");
      } else if (data.type === "request-status") {
        signalingChannel?.send(JSON.stringify({ type: "mute-status", isMuted: isMuted }));
        ...
        console.log(`Пацан під кодовим номером ${data.from} мікро перемкнув на ${data.isMuted}`);
      } else if (data.type === "leave") {
        console.log(`❌ Користувач ${data.from} вийшов (сам сказав), закриваємо з'єднання`);
        playSound("goodbye")
  ...
  useEffect(() => {
    if (localStream) {
        console.log("localStream успішно ініціалізовано! Запускаєм підключення до сокетів")
        init();
  ...
  const init = async () => {
    if (selectedChannel) {
      ...
      const wsUrl = `${protocol}://localhost:8000/ws/voice/${selectedChannel.id}/?token=${token}`;
  ...
  */
  ...
  const [newChannelParticipantsField, setNewChannelParticipantsField] = useState('');
  ...
  const updateUsers = async () => {
    const existingUsersMap = new Map(participants.map((user) => [user.id, user]));
  ...
  const handleAddUser = useCallback(() => {
    if (newChannelParticipantsField.trim()) {
      setNewChannelParticipants(prev => {
          ...
          if (userData && userData.display_name && userData.display_name === newChannelParticipantsField) {
            showError("Ви автоматично станете учасником, як отець войсу!", "warning");
            return prev;
        }
  ...
  const handleCreateChannel = async () => {
    if (newChannelName.trim()) {
      ...
      const createChannelRequest = await apiRequest("/voice_channels/voice_channels/", "POST", JSON.stringify(newChannel))
  ...
  const handleDeleteChannel = async (id : number) => {
    const deleteChannelRequest = await apiRequest(`/voice_channels/voice_channels/${id}/`, "DELETE")
              ...
              <div className='mb-4'>
                ...
                <div className='flex'>
                  <input
                    ...
                    onChange={(e) => setNewChannelParticipantsField(e.target.value)}
                    placeholder='Користувач'
                    className='mb-2 w-full rounded bg-[#25252B] p-2 text-white'
                  />
                  <button
                    onClick={handleAddUser}
                    className='flex-1 rounded bg-[#6374B6] p-1 hover:bg-opacity-70 mb-2 ml-2'
                  >
                    <i className="fa-solid fa-plus text-[#25252B]"></i>
                </button>
                </div>
                {newChannelParticipants.map((participant, key) => (
                  <div key={key} className='flex mb-2 w-full rounded bg-[#25252B] p-2 text-white'>
                    <p className='w-full'>{participant}</p>
                    <button onClick={() => setNewChannelParticipants((prev) => prev.filter((elmt, elmt_key) => elmt_key !== key))}>
                ...
                <div className='flex' key={key}>
                  <motion.button
                    ...
                    className={`flex w-full items-center justify-between rounded-lg p-3 ${
                      selectedChannel && selectedChannel.id && selectedChannel.id === channel.id
                        ? 'bg-[#6374B6]'
                  ...
                  {userData && channel.creator && channel.id && userData.display_name === channel.creator.display_name && 
                    <button
                      onClick={(e) => handleDeleteChannel(channel.id)}
                      className='bg-[#6374B6] rounded p-2 ml-2 hover:bg-opacity-70'
                    >
              ...
              {participants.map((user, key) => (
                ...
                >
                  <div className='relative'>
                    <div
                      style={{borderColor: user.talks? numberToGrayHex(user.talks * 10, "00") : "#000000",
                              boxShadow: `0 4px 20px rgba(0, ${user.talks? user.talks * 10 : 0}, 0, 0.75)`
                            }}
                      className={`h-20 w-20 rounded-full bg-[#2D2D35] overflow-hidden border-4`}
                    >
                  ...
                  {user.photo && (
                    <motion.div
                      style={{borderColor: `${user.talks? numberToGrayHex(user.talks * 10, "AA") : "#4A004A"}`}}
          ...
          {selectedChannel && (
            ...
            <div className='flex items-center gap-4 rounded-full bg-[#25252B] p-2'>
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className='rounded-full p-4 bg-red-500 transition-colors'
                onClick={() => {
                  endCall()
                  playSound("end")
                }}
